...
def fit_single_ticker(df, model, p0=None):
    ...
    max_nfev = 20000  # Reduced for speed
    ...
    for option_type in ['Call', 'Put']:
        ...
        if len(df_type) < 10:
            ...
            metrics_df = pd.DataFrame({
                ...
                'ATM_IV': [atm_iv],
                'Best_Exp_Max_Short': [best_exp_max_short if 'best_exp_max_short' in locals() else np.nan],
                'Best_Exp_Min_Long': [best_exp_min_long if 'best_exp_min_long' in locals() else np.nan]
            })
...
def process_volumes(timestamp):
    ...
    if all_params:
        ...
        print(f"All volatility surface parameters saved to {vol_surf_file}")
